...
class MailTemplate(models.Model):
    ...
    def _qweb_render_context(self, record):
        ...
        """
        res = {"object": record, "email_template": self}
        ...
        if record._name == "mail.message":
            ...
            # in `partner._notify`.
            ctx.update(
                partner_model._notify_prepare_template_context(record)
            )  # pragma: no cover
        res.update(
            {
                ...
                "format_tz":
                    lambda dt, tz=False, format=False, context=self._context: format_tz(
